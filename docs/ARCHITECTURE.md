...
- The pair travels end to end. `FiscalTransaction` carries `vatDeductionPercent` beside `deductionPercent` (already resolved by the view), and `FiscalExpenseTable` keeps its single `Deduc%` column and badges **only** the rows where the two shares disagree — a permanent second column would repeat the first one on almost every row and be tuned out before reaching the one that matters. `RecurringExpenseForm` closes the other end: a rule can be given its own IVA share instead of only inheriting the category default, which matters because the expenses the two shares differ on (internet, luz, calefacción) are recurring.
